...
namespace MTK.FigmaUIImport
{
    ...
    [InitializeOnLoad]
    internal static class MtkFuiImportPackageSelfRepair
    {
        private static bool _scheduled;

        static MtkFuiImportPackageSelfRepair()
        {
            ScheduleCleanup();
        }
        ...
        private static void ScheduleCleanup()
        {
            ...
            _scheduled = true;
            EditorApplication.delayCall += CleanupStaleAsmdefs;
        }

        private static void CleanupStaleAsmdefs()
        {
            try
            {
                var deleted = false;
                var packageAssetPath = "Packages/com.mtk.fui-import/package.json";
                var resolvedRoot = string.Empty;
#if UNITY_2019_1_OR_NEWER
                var packageInfo = UnityEditor.PackageManager.PackageInfo.FindForAssetPath(packageAssetPath);
                if (packageInfo != null) resolvedRoot = packageInfo.resolvedPath;
#endif
                if (string.IsNullOrEmpty(resolvedRoot))
                {
                    resolvedRoot = Path.Combine(AiFuiImporterUtility.ProjectRoot, "Packages/com.mtk.fui-import");
                }

                deleted |= DeleteFileIfExists(Path.Combine(resolvedRoot, "Editor/AI.MultiToolKit.FUIImporter.Editor.asmdef"));
                deleted |= DeleteFileIfExists(Path.Combine(resolvedRoot, "Editor/AI.MultiToolKit.FUIImporter.Editor.asmdef.meta"));
                deleted |= DeleteFileIfExists(Path.Combine(resolvedRoot, "Editor/MTK.FigmaUIImport.Editor.asmdef"));
                deleted |= DeleteFileIfExists(Path.Combine(resolvedRoot, "Editor/MTK.FigmaUIImport.Editor.asmdef.meta"));

                // Also try AssetDatabase paths. This helps when Unity exposes the package as an embedded Packages/ folder.
                deleted |= AssetDatabase.DeleteAsset("Packages/com.mtk.fui-import/Editor/AI.MultiToolKit.FUIImporter.Editor.asmdef");
                deleted |= AssetDatabase.DeleteAsset("Packages/com.mtk.fui-import/Editor/AI.MultiToolKit.FUIImporter.Editor.asmdef.meta");
                deleted |= AssetDatabase.DeleteAsset("Packages/com.mtk.fui-import/Editor/MTK.FigmaUIImport.Editor.asmdef");
                deleted |= AssetDatabase.DeleteAsset("Packages/com.mtk.fui-import/Editor/MTK.FigmaUIImport.Editor.asmdef.meta");

                if (deleted)
                {
                    Debug.Log("[MTK | Figma UI Import] Removed stale assembly definition files from previous package versions. Refreshing AssetDatabase.");
                    AssetDatabase.Refresh();
                }
            }
            catch (Exception ex)
            {
                Debug.LogWarning("[MTK | Figma UI Import] Could not auto-remove stale asmdef files. Delete old Packages/com.mtk.fui-import/Editor/*.asmdef files manually. Reason: " + ex.Message);
            }
        }

        private static bool DeleteFileIfExists(string absolutePath)
        {
