...
namespace AiMultiToolKit.FuiImporter
{
    ...
    public static class AiFuiImporter
    {
        ...
        public static FuiImportReport Import(string fuiFilePath, FuiImportOptions options)
        {
            ...
            var runtimeRoot = AiFuiImporterUtility.CombineAssetPath(outputRoot, "Runtime");
            ...
            AiFuiImporterUtility.EnsureAssetFolder(runtimeRoot);
            WriteFuiGeneratedRuntimeSupport(runtimeRoot);
        ...
        private static void WriteFuiGeneratedRuntimeSupport(string runtimeRoot)
        {
            var path = AiFuiImporterUtility.CombineAssetPath(runtimeRoot, "FuiGradientText.cs");
            var sb = new StringBuilder();
            sb.AppendLine("// Auto-generated by AI Multi-Tool Kit FUI Importer. Keep this file with imported UXML assets.");
            sb.AppendLine("using UnityEngine.UIElements;");
            sb.AppendLine();
            sb.AppendLine("namespace AiMultiToolKit.Generated");
            sb.AppendLine("{");
            sb.AppendLine("    internal static class FuiRichTextUtility");
            sb.AppendLine("    {");
            sb.AppendLine("        public static string ComposeGradientText(string plainText, string gradientName)");
            sb.AppendLine("        {");
            sb.AppendLine("            var source = plainText ?? string.Empty;");
            sb.AppendLine("            if (string.IsNullOrEmpty(gradientName)) return source;");
            sb.AppendLine("            var safeGradientName = (gradientName ?? string.Empty).Trim();");
            sb.AppendLine("            if (string.IsNullOrEmpty(safeGradientName)) return source;");
            sb.AppendLine("            var safeText = EscapeNoparseMarkers(source);");
            sb.AppendLine("            return \"<color=#FFFFFFFF><gradient=\\\"\" + safeGradientName + \"\\\"><noparse>\" + safeText + \"</noparse></gradient></color>\";");
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        private static string EscapeNoparseMarkers(string value)");
            sb.AppendLine("        {");
            sb.AppendLine("            if (string.IsNullOrEmpty(value)) return string.Empty;");
            sb.AppendLine("            return value.Replace(\"<noparse>\", \"<no\\u200Bparse>\").Replace(\"</noparse>\", \"</no\\u200Bparse>\");");
            sb.AppendLine("        }");
            sb.AppendLine("    }");
            sb.AppendLine();
            sb.AppendLine("    public class FuiGradientLabel : Label");
            sb.AppendLine("    {");
            sb.AppendLine("        public new class UxmlFactory : UxmlFactory<FuiGradientLabel, UxmlTraits> { }");
            sb.AppendLine("        public new class UxmlTraits : Label.UxmlTraits");
            sb.AppendLine("        {");
            sb.AppendLine("            private readonly UxmlStringAttributeDescription _plainText = new UxmlStringAttributeDescription { name = \"plain-text\", defaultValue = \"\" };");
            sb.AppendLine("            private readonly UxmlStringAttributeDescription _gradientName = new UxmlStringAttributeDescription { name = \"gradient-name\", defaultValue = \"\" };");
            sb.AppendLine();
            sb.AppendLine("            public override void Init(VisualElement ve, IUxmlAttributes bag, CreationContext cc)");
            sb.AppendLine("            {");
            sb.AppendLine("                base.Init(ve, bag, cc);");
            sb.AppendLine("                var label = (FuiGradientLabel)ve;");
            sb.AppendLine("                label._plainText = _plainText.GetValueFromBag(bag, cc) ?? string.Empty;");
            sb.AppendLine("                label._gradientName = _gradientName.GetValueFromBag(bag, cc) ?? string.Empty;");
            sb.AppendLine("                label.ApplyFuiGradientText();");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        private string _plainText = string.Empty;");
            sb.AppendLine("        private string _gradientName = string.Empty;");
            sb.AppendLine();
            sb.AppendLine("        public string PlainText");
            sb.AppendLine("        {");
            sb.AppendLine("            get { return _plainText; }");
            sb.AppendLine("            set { _plainText = value ?? string.Empty; ApplyFuiGradientText(); }");
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        public string GradientName");
            sb.AppendLine("        {");
            sb.AppendLine("            get { return _gradientName; }");
            sb.AppendLine("            set { _gradientName = value ?? string.Empty; ApplyFuiGradientText(); }");
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        public new string text");
            sb.AppendLine("        {");
            sb.AppendLine("            get { return _plainText; }");
            sb.AppendLine("            set { _plainText = value ?? string.Empty; ApplyFuiGradientText(); }");
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        public FuiGradientLabel()");
            sb.AppendLine("        {");
            sb.AppendLine("            enableRichText = true;");
            sb.AppendLine("            RegisterCallback<AttachToPanelEvent>(_ => ApplyFuiGradientText());");
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        private void ApplyFuiGradientText()");
            sb.AppendLine("        {");
            sb.AppendLine("            enableRichText = true;");
            sb.AppendLine("            base.text = FuiRichTextUtility.ComposeGradientText(_plainText, _gradientName);");
            sb.AppendLine("            MarkDirtyRepaint();");
            sb.AppendLine("        }");
            sb.AppendLine("    }");
            sb.AppendLine();
            sb.AppendLine("    public class FuiGradientButton : Button");
            sb.AppendLine("    {");
            sb.AppendLine("        public new class UxmlFactory : UxmlFactory<FuiGradientButton, UxmlTraits> { }");
            sb.AppendLine("        public new class UxmlTraits : Button.UxmlTraits");
            sb.AppendLine("        {");
            sb.AppendLine("            private readonly UxmlStringAttributeDescription _plainText = new UxmlStringAttributeDescription { name = \"plain-text\", defaultValue = \"\" };");
            sb.AppendLine("            private readonly UxmlStringAttributeDescription _gradientName = new UxmlStringAttributeDescription { name = \"gradient-name\", defaultValue = \"\" };");
            sb.AppendLine();
            ...
            sb.AppendLine("            {");
            sb.AppendLine("                base.Init(ve, bag, cc);");
            sb.AppendLine("                var button = (FuiGradientButton)ve;");
            sb.AppendLine("                button._plainText = _plainText.GetValueFromBag(bag, cc) ?? string.Empty;");
            sb.AppendLine("                button._gradientName = _gradientName.GetValueFromBag(bag, cc) ?? string.Empty;");
            sb.AppendLine("                button.ApplyFuiGradientText();");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        private string _plainText = string.Empty;");
            ...
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        public FuiGradientButton()");
            sb.AppendLine("        {");
            sb.AppendLine("            enableRichText = true;");
            sb.AppendLine("            RegisterCallback<AttachToPanelEvent>(_ => ApplyFuiGradientText());");
            sb.AppendLine("        }");
            ...
            sb.AppendLine("    }");
            sb.AppendLine("}");
            WriteTextAsset(path, sb.ToString());
        }
    ...
    internal sealed class FuiUiToolkitGenerator
    {
        ...
        private static readonly Regex TextCoreRichTextTagPattern = new Regex(@"</?\s*(?:a|align|allcaps|alpha|b|br|color|cspace|font|font-weight|gradient|i|indent|line-height|line-indent|link|lowercase|margin|mark|mspace|nobr|noparse|pos|s|size|smallcaps|space|sprite|style|sub|sup|u|uppercase|voffset|width)\b[^>]*>", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
        ...
        public FuiGeneratedScreen Generate()
        {
            ...
            uxml.AppendLine("<ui:UXML xmlns:ui=\"UnityEngine.UIElements\" xmlns:fui=\"AiMultiToolKit.Generated\" editor-extension-mode=\"False\">");
        ...
        private string BuildElementXml(Dictionary<string, object> element, int indent, bool isRoot, string parentLayoutMode, Dictionary<string, object> parentBounds)
        {
            ...
            var text = SanitizeImportedText(!string.IsNullOrWhiteSpace(ownText) ? ownText : FindVisibleText(element));
            var gradient = FuiJson.GetObject(style, "textGradient");
            var hasTextGradient = gradient != null;
            var gradientPresetName = hasTextGradient ? EnsureTextGradientPreset(gradient, name) : string.Empty;
            if (type == "Label" && !textAsImage)
            {
                if (!string.IsNullOrEmpty(gradientPresetName))
                {
                    tag = "fui:FuiGradientLabel";
                    attrs.Append(" plain-text=\"").Append(XmlEscape(text)).Append("\"");
                    attrs.Append(" gradient-name=\"").Append(XmlEscape(gradientPresetName)).Append("\"");
                }
                else
                {
                    attrs.Append(" text=\"").Append(XmlEscape(text)).Append("\"");
                }
            }
        ...
        {
            // Kept as a safe fallback for older call paths: never inject rich-text tags into
            // a normal UI Toolkit text attribute. Gradient text is handled through
            // FuiGradientLabel/FuiGradientButton plain-text + gradient-name attributes.
            return SanitizeImportedText(text ?? string.Empty);
        }
        ...
        private static string SanitizeImportedText(string value)
        {
            if (string.IsNullOrEmpty(value)) return string.Empty;
            value = value.Replace("\r\n", "\n").Replace("\r", "\n");
            return TextCoreRichTextTagPattern.Replace(value, string.Empty);
        }
